...
export function SettingsPanel({ onClose }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex animate-fade-in justify-end">
      ...
      {/* Panel — C2: opaque dark bg so underlying content doesn't bleed through (blur helps in real WebView2) */}
      <div className="relative m-2 flex w-[280px] flex-col overflow-y-auto scrollbar-thin rounded-2xl border border-white/[0.1] bg-black/90 p-4 backdrop-blur-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-white/85">设置</h2>
          <button
            onClick={onClose}
            // M5: 44x44px touch target
            className="flex h-8 w-8 items-center justify-center rounded-md text-white/50 hover:bg-white/10 hover:text-white/80"
            aria-label="关闭设置"
          >
            <X size={16} />
          </button>
...
function Section({
  ...
  return (
    // L4: explicit divider + spacing so sections are scannable
    <div className="mb-4 border-t border-white/[0.06] pt-3 first:border-t-0 first:pt-0">
      <div className="mb-2 flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-white/50">
        {icon}
        {title}
      </div>
