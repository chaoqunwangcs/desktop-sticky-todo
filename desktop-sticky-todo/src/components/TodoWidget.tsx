...
export function TodoWidget() {
  ...
  // H6: KeyboardSensor enables keyboard reorder (Space to pick, arrows to move)
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 5 } }),
    // H6: keyboard users can reorder via space/enter + arrows
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Active list — C1: force scrollbar visible so overflow is discoverable */}
      <div className="scrollbar-thin -mx-1 flex-1 overflow-y-scroll px-1">
        {active.length === 0 ? (
          // M2: helpful empty state with onboarding hint
          <div className="flex h-32 flex-col items-center justify-center gap-1.5 text-white/40">
            <Inbox size={24} />
            <span className="text-xs font-medium">暂无待办</span>
            <span className="text-[10px] text-white/30">在上方输入框输入内容，回车添加</span>
          </div>
        ) : (
          <DndContext
            sensors={sensors}
        ...
        {done.length > 0 && (
          <div className="mt-3">
            <button
              ...
              className="flex w-full items-center gap-1.5 px-1 py-1 text-xs text-white/55 hover:text-white/75"
            >
              {showArchive ? (
                <ChevronDown size={13} />
              ) : (
            ...
            {showArchive && (
              // C3: no opacity-60 stacking — TodoItem handles its own done styling
